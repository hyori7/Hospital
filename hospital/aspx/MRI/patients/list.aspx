...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
	<script type="text/javascript">
		function view(pId, testId, historyId) {
			document.location.href = "/hospital/aspx/MRI/MRI/create.aspx?pId=" + pId + "&testId=" + testId + "&historyId=" + historyId;
		}

		function done(hId) {
			var check = confirm("Have you done every test for this Patient?");
			if (!check) return;
			document.location.href = "/hospital/aspx/MRI/patients/list.aspx?hId=" + hId;
		}
	</script>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cnt" runat="server">
	...
	<select name="searchField" id="searchField" class="inputStyle" onfocus="changeColour(this, '#ccc')">
	...
	<input id="searchValue" name="searchValue" value="<%=Param.get("searchValue")%>" onfocus="changeColour(this, '#ccc')" class="inputStyle"/>
	 DATE : <asp:TextBox ID="date" runat="server" CssClass="inputStyle"></asp:TextBox> 
	&nbsp;<asp:Button ID="search" runat="server" Text="SEARCH" OnClick="onSearch" CssClass="inputStyle" style="cursor:pointer;"
 onfocus="changeColour(this, '#ccc')"/>
