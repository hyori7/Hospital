...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.rangeYearUpper = 2010;
	this.minDate = new Date(1900,0,1);
	this.maxDate = new Date(2011,0,1);
...
Epoch.prototype.outputDate = function (vDate, vFormat) //PUBLIC: outputs a date in the appropriate format (DEPRECATED)
{
	...
	//alert(vFormat.replace(/mm/g, vDay).replace(/dd/g, vMonth).replace(/y{1,4}/g, vYear).replace(/hh/g, vHour).replace(/nn/g, vMinute).replace(/ss/g, vSecond));
	//return vFormat.replace(/mm/g, vDay).replace(/dd/g, vMonth).replace(/y{1,4}/g, vYear).replace(/hh/g, vHour).replace(/nn/g, vMinute).replace(/ss/g, vSecond);
};
//-----------------------------------------------------------------------------
	...
	if(!owner.selCurMonthOnly || cell.date.getMonth() == owner.displayMonth && cell.date.getFullYear() == owner.displayYear)
	{
		...
		else //if we can only select one cell at a time
		{
			...
			if(owner.tgt) //if there is a target element to place the value in, do so
			{
				...
				var dayArray = owner.tgt.value.split("/");
				owner.tgt.value = dayArray[1] + "/" + dayArray[0] + "/" + dayArray[2];
